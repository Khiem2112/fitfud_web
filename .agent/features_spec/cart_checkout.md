...
- Spec popup liên quan: `popup/cart_hover.md`, `popup/saved_address.md`, `popup/add_address.md`, `popup/order_success.md`, `popup/otp_order_confirm.md`, `popup/error.md`
...
- Khi mở từ Header, popup có 2 trạng thái: hover tạm thời và click để ghim; chi tiết tại `popup/cart_hover.md`.
...
- Popup chọn địa chỉ đã lưu và thêm địa chỉ mới được mô tả tại `popup/saved_address.md` và `popup/add_address.md`.
...
### Popup sau khi đặt hàng
- Nếu tạo đơn thành công, hiển thị Popup đặt đơn thành công theo `popup/order_success.md`.
- Nếu luồng thanh toán hoặc xác nhận đơn yêu cầu OTP, sử dụng Popup OTP tại `popup/otp_order_confirm.md`.
- Nếu tạo đơn thất bại hoặc thao tác không hợp lệ, hiển thị Popup lỗi dùng chung tại `popup/error.md`.
